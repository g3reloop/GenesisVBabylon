      ...
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center py-4">
          ...
          <div className="lg:hidden">
            <button className="text-emerald-100 hover:text-emerald-400 mobile-tap-target p-2">
          ...
          <div className="px-2 pt-2 pb-3 space-y-1">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className={`block px-3 py-2 text-base font-medium transition-colors mobile-tap-target ${
                  pathname === item.href
                    ? 'text-emerald-400 bg-emerald-800'
                    : 'text-emerald-100 hover:text-emerald-400 hover:bg-emerald-800'
                }`}
              >
                {item.label}
              </Link>
